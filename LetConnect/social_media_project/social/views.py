...
def index(request):
    user_instance = get_object_or_404(User, username='nithin')
        ...
        if request.method == 'PUT':
            try:
                ...
                if request.user == post.creater:
                    ...
                    return HttpResponse(status=403)  # Forbidden, user is not the creator
            except Post.DoesNotExist:
                return HttpResponse(status=404)  # Post not found
            except Exception as e:
                return HttpResponse(status=500, content=f"Error: {str(e)}")
        else:
            return HttpResponse(status=405, content="Method Not Allowed")  # Only allow PUT requests
    else:
        return HttpResponseRedirect(reverse('login'))
